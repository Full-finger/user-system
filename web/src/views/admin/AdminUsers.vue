  ...
  <div class="admin-users">
    ...
    <div class="card admin__table-wrap">
      ...
      <div v-else-if="users.length === 0" class="empty-state">
        <div class="empty-state__icon"><PhUsers :size="24" /></div>
...
async function handleEdit() {
  ...
  if (editForm.role !== editModal.user.role) data.role = editForm.role
  if (Object.keys(data).length === 0) { editModal.show = false; return }
...
.admin__role-options { display: flex; flex-wrap: wrap; gap: 8px; }
...
@media (max-width: 600px) {
  ...
  .admin__table td:nth-child(2) { display: none; }
}
</style>